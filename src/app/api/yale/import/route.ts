...
import { getYaleObjectDetail, getYaleImageUrl } from "@/lib/yale";
...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const item of items) {
      ...
      try {
        const detail = await getYaleObjectDetail(item.objectId);
        ...
        // Fetch full-res image from IIIF Image Service
        const imageUrl = getYaleImageUrl(detail.imageServiceUrl);
